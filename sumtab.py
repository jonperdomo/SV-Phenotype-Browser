...
def sumtab(phenotype):
    ...
    cur = conn.cursor()
    cur.execute('SELECT CHROM, SV_START, SV_END, PHENOTYPE FROM sv_phenotypes WHERE phenotype=?',
                [phenotype])  # filters through the databased according to the input phenotype from the php.
